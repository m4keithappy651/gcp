...
# ==============================================
#        LOW MS OPTIMIZER - 24/7 BACKGROUND
#        ZERO QUOTA - CLIENT SIDE ONLY
# ==============================================
low_ms_optimizer() {
    local TARGET="${CLEAN_HOST}"
    local LOG_DIR="/tmp/gcp-optimizer"
    local PID_FILE="$LOG_DIR/optimizer.pid"
    local LOG_FILE="$LOG_DIR/latency-$(date +%Y%m%d-%H%M%S).log"
    
    mkdir -p "$LOG_DIR"
    
    echo ""
    echo -e "${C_HEADER}════════════════════════════════════════════════════════════════════════════${RESET}"
    echo -e "${C_PLAIN}$(math_bold "LOW MS OPTIMIZER - 24/7 BACKGROUND")${RESET}"
    echo -e "${C_HEADER}════════════════════════════════════════════════════════════════════════════${RESET}"
    echo ""
    echo -e "${C_INFO}[*]${RESET} Target: ${BOLD}${TARGET}${RESET}"
    echo -e "${C_INFO}[*]${RESET} Optimizations: TCP Fast Open, BBR, Keep-Alive, DNS Prefetch, Connection Pooling"
    echo -e "${C_INFO}[*]${RESET} Zero GCP quota - Client-side only"
    echo ""
    
    cat > /tmp/low-ms-optimizer.sh <<'OPTIMIZER_EOF'
#!/bin/bash
TARGET="$1"
LOG_FILE="$2"
PID_FILE="$3"
...
grep -q bbr /proc/sys/net/ipv4/tcp_available_congestion_control 2>/dev/null && echo bbr > /proc/sys/net/ipv4/tcp_congestion_control 2>/dev/null || true
echo 60 > /proc/sys/net/ipv4/tcp_keepalive_time 2>/dev/null || true
...
echo "========================================="
echo "   LOW MS OPTIMIZER ACTIVE (24/7)"
echo "   Target: $TARGET"
echo "   Log: $LOG_FILE"
echo "   PID: $$"
echo "========================================="
LATENCY_SUM=0; LATENCY_COUNT=0; LATENCY_MIN=9999; LATENCY_MAX=0
while true; do
    TIMESTAMP=$(date '+%Y-%m-%d %H:%M:%S')
    dig +short "$TARGET" > /dev/null 2>&1 || true
    RESULT=$(curl -o /dev/null -s -w '%{time_connect}|%{time_total}|%{http_code}' --max-time 5 "https://$TARGET/health" 2>/dev/null)
    if [ -n "$RESULT" ]; then
        TIME_CONNECT=$(echo "$RESULT" | cut -d'|' -f1)
        TIME_TOTAL=$(echo "$RESULT" | cut -d'|' -f2)
        HTTP_CODE=$(echo "$RESULT" | cut -d'|' -f3)
        LATENCY_MS=$(echo "$TIME_CONNECT * 1000" | bc 2>/dev/null | cut -d'.' -f1)
        [ -z "$LATENCY_MS" ] && LATENCY_MS=$(echo "$TIME_TOTAL * 1000" | bc 2>/dev/null | cut -d'.' -f1)
        if [ -n "$LATENCY_MS" ]; then
            LATENCY_SUM=$((LATENCY_SUM + LATENCY_MS)); LATENCY_COUNT=$((LATENCY_COUNT + 1))
            [ "$LATENCY_MS" -lt "$LATENCY_MIN" ] && LATENCY_MIN=$LATENCY_MS
            [ "$LATENCY_MS" -gt "$LATENCY_MAX" ] && LATENCY_MAX=$LATENCY_MS
            AVG_LATENCY=$((LATENCY_SUM / LATENCY_COUNT))
        fi
        if [ "$HTTP_CODE" = "200" ]; then
            [ "$LATENCY_MS" -lt 100 ] && STATUS="EXCELLENT" && COLOR="\033[0;32m" || { [ "$LATENCY_MS" -lt 200 ] && STATUS="GOOD" && COLOR="\033[1;33m" || { STATUS="HIGH" && COLOR="\033[0;31m"; }; }
            echo -e "[$TIMESTAMP] ${COLOR}UP${RESET} | ${LATENCY_MS}ms | $STATUS | Avg: ${AVG_LATENCY}ms | Min: ${LATENCY_MIN}ms | Max: ${LATENCY_MAX}ms"
            echo "[$TIMESTAMP] UP | ${LATENCY_MS}ms | $STATUS | Avg: ${AVG_LATENCY}ms" >> "$LOG_FILE"
        else
            echo -e "[$TIMESTAMP] \033[1;33mWARN${RESET} | HTTP $HTTP_CODE"
            echo "[$TIMESTAMP] WARN | HTTP $HTTP_CODE" >> "$LOG_FILE"
        fi
    else
        echo -e "[$TIMESTAMP] \033[0;31mDOWN${RESET} | No response"
        echo "[$TIMESTAMP] DOWN | No response" >> "$LOG_FILE"
    fi
    curl -s -o /dev/null --keepalive-time 60 "https://$TARGET/health" 2>/dev/null &
    [ $(( $(date +%s) % 300 )) -lt 5 ] && dig +short "$TARGET" > /dev/null 2>&1 &
    sleep 5
done
OPTIMIZER_EOF

    chmod +x /tmp/low-ms-optimizer.sh
    nohup /tmp/low-ms-optimizer.sh "$TARGET" "$LOG_FILE" "$PID_FILE" > /dev/null 2>&1 &
    local OPTIMIZER_PID=$!
    sleep 1
    
    if kill -0 $OPTIMIZER_PID 2>/dev/null; then
        echo -e "${C_SUCCESS}╔════════════════════════════════════════════════════════════════════════════╗${RESET}"
        echo -e "${C_SUCCESS}║${RESET}                    ${BOLD}${GREEN}LOW MS OPTIMIZER STARTED${RESET}                            ${C_SUCCESS}║${RESET}"
        echo -e "${C_SUCCESS}╠════════════════════════════════════════════════════════════════════════════╣${RESET}"
        echo -e "${C_SUCCESS}║${RESET}  ${CYAN}PID:${RESET}        ${BOLD}$OPTIMIZER_PID${RESET}"
        echo -e "${C_SUCCESS}║${RESET}  ${CYAN}Log File:${RESET}   ${BOLD}$LOG_FILE${RESET}"
        echo -e "${C_SUCCESS}║${RESET}  ${CYAN}Target:${RESET}     ${BOLD}$TARGET${RESET}"
        echo -e "${C_SUCCESS}║${RESET}  ${CYAN}Optimizations:${RESET} TCP Fast Open, BBR, Keep-Alive, DNS Prefetch      ${C_SUCCESS}║${RESET}"
        echo -e "${C_SUCCESS}║${RESET}                                                                            ${C_SUCCESS}║${RESET}"
        echo -e "${C_SUCCESS}║${RESET}  ${CYAN}View log:${RESET}  ${GREEN}tail -f $LOG_FILE${RESET}${C_SUCCESS}║${RESET}"
        echo -e "${C_SUCCESS}║${RESET}  ${CYAN}Stop:${RESET}      ${GREEN}kill $OPTIMIZER_PID${RESET}${C_SUCCESS}║${RESET}"
        echo -e "${C_SUCCESS}╚════════════════════════════════════════════════════════════════════════════╝${RESET}"
        echo ""
        echo -e "${C_WARN}[!]${RESET} Optimizer runs 24/7. Zero GCP quota used."
    else
